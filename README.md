...
- Upstream PiClaw PR work happens from `/workspace/src/piclaw-fork` with `origin` on the personal fork and `upstream` on `rcarmo/piclaw`
...
- Create a clean fork checkout at `/workspace/src/piclaw-fork` for upstream PR work
...
`/workspace/src/piclaw-live` is the deployment checkout and may carry local patch-stack state. Do upstream PR work from `/workspace/src/piclaw-fork` instead.
...
- **Restic to Cloudflare R2** — daily encrypted backups of `/workspace` to the `pix-backup` R2 bucket, excluding rebuildable paths such as `/workspace/.cache`, `/workspace/src/piclaw-live*`, and `/workspace/src/piclaw-fork`, with 7 daily / 4 weekly / 3 monthly retention. Configured in `modules/backup.nix`
