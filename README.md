...
NixOS host configuration for `pix2`, the Hermes agent host.
...
The `pix2` target runs:

- Base OS on NixOS `25.11`, with `nix-command`/flakes, daily GC retaining four system generations, automatic store optimisation, disk-pressure GC guard, zram, 8 GiB swapfile, latest kernel from `kernel-nixpkgs`, journald/coredump limits, and common CLI tooling.
...
- CLIProxyAPI on loopback for Hermes and AgentMemory model access.
...
| `80/tcp`, `443/tcp` | Tailscale only | Reserved tailnet web entrypoints |
| `8787/tcp` | Local | Hermes WebUI |
...
- shell aliases for host jobs, Hermes, Claude, Codex, tmux, and zellij.
...
package pins for AgentMemory, Amp, Camofox Browser, Claude Code ACP,
CLIProxyAPI, Codex ACP, Cursor CLI, Droid, Gog, iii, Oracle, Portless, SFW,
signal-cli, Tirith, and Vet, then validates the exported package set with
`nix build`.
...
- Hermes, Hermes WebUI, and Camofox checkouts are mutable runtime/source checkouts under `/workspace/src`.
